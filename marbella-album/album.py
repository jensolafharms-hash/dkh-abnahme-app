                    ...
                    for i, st in enumerate(stepsb):
                        ...
                        if P.get("bass_inst", "synth") == "pluck":
                            ...
                            place(self.layers["bass"], at(st),
                                  synth_bass(rng, float(midi_to_hz(m_)), int(self.step_len(cur, nxt - st) * (0.65 if bass in ("roll", "deep") else 0.92)),
                                             level=blvl * (1.0 if st in (0, 8, 12) else 0.85), cutoff=P.get("bass_cut", 600.0)))
...
TRACKS = [
    ...
    dict(nr=2, title="La Concha Horizon", seed=2202, root=55, mode="G-Moll, andalusische Kadenz", style="Deep House",
         scale=[0, 2, 3, 5, 7, 8, 10], themes=T_CONCHA,
         progs=dict(verse=["i", "VII", "VI", "V"], refrain=["VI", "VII", "i", "V7"], bridge=["iv7", "V", "iv7", "V"], climax=["VI7", "VII", "i9", "V7"]),
         sections=[
             S("intro", 8, bpm=120, dyn=0.64, pad=700, bass="long", bass_level=0.55, bass_cut=400.0,
               melody=[mel("frag", "mtrumpet", 0.22, legato=1.3, humanize=0.02)], atmos=dict(crickets=0.3, cicadas=0.2)),
             S("deep1", 24, bpm=120, dyn=0.78, drums="deep", drum_level=0.85, hats="offbeat", bass="deep", bass_level=0.75, bass_cut=520.0,
               comping="rumba", comp_level=0.12, pad=900, perc=["shaker"], melody=[mel("A", "mtrumpet", 0.24)], answer="guitar", answer_level=0.12),
             S("build", 8, bpm=120, dyn=0.85, drums="deep", drum_level=0.9, hats="full", bass="deep", bass_level=0.8, bass_cut=560.0, acid=0.18, acid_cut=300.0,
               comping="rumba", comp_level=0.14, pad=1100, perc=["shaker", "palmas"], perc_level=0.7, fill=True,
               melody=[mel("A", "guitar", 0.36, ornaments=True)]),
             S("chorus1", 16, bpm=120, prog="refrain", dyn=0.92, drums="house", drum_level=0.95, hats="full", bass="roll", bass_level=0.8, bass_cut=650.0,
               comping="rumba", comp_level=0.15, pad=1400, stabs="brass", stab_steps=(2, 10), stab_level=0.17, perc=["palmas", "shaker"], acid=0.2, acid_cut=340.0, fill=True,
               melody=[mel("B", "trumpet", 0.26), mel("B", "guitar", 0.26, octave=0, pan=0.3, legato=0.8)]),
             S("brass_break", 8, bpm=120, prog="refrain", dyn=0.82, drums="kickonly", drum_level=0.9, bass="roll", bass_level=0.8, bass_cut=650.0,
               stabs="brass", stab_steps=(2, 5, 10, 13), stab_level=0.24, perc=["palmas"], pad=1000),
             S("deep2", 16, bpm=120, dyn=0.8, drums="deep", drum_level=0.85, hats="offbeat", bass="deep", bass_level=0.75, bass_cut=520.0, acid=0.18, acid_cut=320.0,
               comping="rumba", comp_level=0.12, pad=900, perc=["shaker", "congas"], melody=[mel("A", "mtrumpet", 0.24)], answer="guitar", answer_level=0.12),
             S("chorus2", 16, bpm=120, prog="refrain", dyn=0.95, drums="house", drum_level=0.95, hats="full", bass="roll", bass_level=0.82, bass_cut=680.0,
               comping="rumba", comp_level=0.15, pad=1450, stabs="brass", stab_steps=(2, 10), stab_level=0.18, perc=["palmas", "shaker", "congas"], acid=0.22, acid_cut=380.0, fill=True,
               melody=[mel("B", "trumpet", 0.26), mel("B", "guitar", 0.26, octave=0, pan=0.3, legato=0.8), mel("A", "steel", 0.12, octave=24, pan=0.5, offset=8, bars=8, legato=0.6)]),
             S("breakdown", 8, bpm=120, prog="bridge", dyn=0.7, bass="long", bass_level=0.55, bass_cut=420.0, pad=800, fx=["riser", "roll"],
               melody=[mel("frag", "mtrumpet", 0.22, legato=1.3, humanize=0.02)], atmos=dict(cicadas=0.5, crickets=0.3)),
             S("chorus3", 16, bpm=120, prog="climax", dyn=1.0, drums="house", drum_level=1.0, hats="full", ride=True, bass="roll", bass_level=0.85, bass_cut=760.0,
               comping="rumba", comp_level=0.16, pad=1600, stabs="brass", stab_steps=(2, 10), stab_level=0.2, perc=["palmas", "shaker", "congas"], acid=0.24, acid_cut=460.0,
               fx=["crash"], fill=True,
               melody=[mel("B", "trumpet", 0.28), mel("B", "guitar", 0.26, octave=0, pan=0.3, legato=0.8), mel("A", "steel", 0.13, octave=24, pan=0.5, offset=8, bars=8, legato=0.6)]),
             S("outro1", 6, bpm=120, dyn=0.7, drums="deep", drum_level=0.7, hats="offbeat", bass="deep", bass_level=0.7, bass_cut=500.0, pad=900,
               melody=[mel("frag", "mtrumpet", 0.2, legato=1.3)]),
             S("outro2", 8, bpm=120, dyn=0.6, pad=700, melody=[mel("frag", "mtrumpet", 0.2, legato=1.4, humanize=0.02)], atmos=dict(crickets=0.4)),
             S("end", 2, bpm=120, dyn=0.55, fx=["final_chord"]),
         ]),
